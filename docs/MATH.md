...
Voxel visibility is conservative, so ingest re-checks each object individually before scoring it. The render distance check compares the object's center distance against the render distance directly, which is the definition of "within render distance" at the object level. It's phrased as a failure of $distance \le renderDistance$ so that a NaN distance (from a NaN position upstream) falls into the cull instead of feeding a NaN priority into the heap, where it would silently corrupt the ordering.
...
The queue dequeues the lowest value first, and the bands from SYSTEM.md section 8 have these exact values. Writing $j$ for the jittered elapsed time since the object's last update, $best$ and $worst$ for the refresh periods, and $u = (j - best)/(worst - best)$ for progress through the refresh window,

| Band | Condition | Priority value | Resulting range |
| --- | --- | --- | --- |
...
| Fast pass | budget ran out | $\min(voxelIndex \times 10^4,\ 8.9 \times 10^5)$ | $[2 \times 10^4,\ 8.9 \times 10^5]$ |
| Over-refreshed | $j \le best$ | $(0.9 + 1 - \min(screenSize, 1)) \times 10^6$ | $[9 \times 10^5,\ 1.9 \times 10^6]$ |

The conditions overlap only at the very nearby band, and the refresh clock wins both ways. An object still inside its best refresh period parks in the over-refreshed tier even while hugging the camera, and one past its worst period takes the p0 value rather than the nearby one.

The tier arithmetic makes the hard separations provable. The scored band tops out at exactly $85 + 13 + 2 = 100$, far below any fast pass tier, so coarsely dumped objects never jump properly scored ones. The dump begins at the voxel after the one that exhausted the budget, which is why the lowest tier that actually occurs is $2 \times 10^4$ rather than the formula's $10^4$. The fast pass cap is one tier under the over-refresh floor, $0.9 \times 10^6 - 10^4 = 8.9 \times 10^5 < 9 \times 10^5$, so even the deepest dumped voxel sorts ahead of every object that was refreshed within its best period, which is the guarantee that fast-passed objects can still update this frame. Voxels from index 89 on share the capped tier, trading relative order among the farthest voxels for that guarantee. The first three bands deliberately interleave at the extremes. A nearby object closer than 27 studs scores under the p0 threshold of $0.9$ and so picks up p0 treatment from the update loop, which is intended, things touching the camera deserve it.
...
The update loop's budget arithmetic closes the loop. While the front of the queue is below the p0 threshold, the iterator allows $1.15\times$ the update budget (0.46 ms on the 0.4 ms default), and with `strictlyEnforceWorstRefreshRate` the p0 allowance becomes a full second, which against frame timescales means unbounded. The dt handed to your callback is the real elapsed time since that object's last update. Each update stamps the object with the clock reading captured when that frame's iteration began, and dt is the difference between this frame's reading and the stamp, so every object in a frame ages against the same instant. Dts of a second or more are excluded from the average-refresh metric since they measure an object coming back into view, not the system's pace.
...
| Fast-pass voxel-tier priorities | Coarsens ordering only | Still ahead of over-refreshed work, still updates this frame |
