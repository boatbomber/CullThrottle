...
so every eroded extent stays positive and the pad never exceeds one voxel. Section 12's proof then says a box fully inside the eroded umbra (a coarse bucket, a search volume, or a single cell, the argument never uses the box's size) has every sightline $[c_0, q]$ passing through the eroded box, and that is exactly the durability we need.
