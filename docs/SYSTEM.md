...
Three terms recur constantly, so here is what we mean by them. Visible means inside the camera's view frustum and within the render distance. Visibility is decided per voxel (a cube of world space, defined in section 4) rather than per object, so it is deliberately a little conservative, and section 8 covers where the per-object refinement happens. Refresh rate is how often a particular object comes back through the update stream. You configure a range (15-60 Hz by default) and CullThrottle moves each object within it. A budget gives each phase a time allowance, checks progress mid-work, and falls back to a defined behavior if time runs out.
