...
```toml
[dependencies]
CullThrottle = "boatbomber/cullthrottle@0.1.0-rc.11"
```
...
The throttle half decides what the visible objects deserve. It scores each one, dominated by how large it looms on screen, with smaller corrections so neglected objects gain urgency and nearby ones get a nudge. The scores feed a priority queue, and your update loop drains it under a time budget. Anything that misses a frame comes back more urgent in the next, and an object overdue past its worst allowed refresh rate jumps to the front of the line. Every visible object keeps updating, update frequency tracks importance, and under sustained pressure the whole system slows down smoothly instead of letting some objects freeze.
